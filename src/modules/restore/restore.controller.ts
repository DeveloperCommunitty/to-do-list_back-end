...
export class RestoreController {
  ...
  @ApiResponse({
    ...
    type: PasswordRedefinition,
  })
  @ApiResponse({ status: 400, description: 'Erro ao criar tarefa' })
  @ApiResponse({ status: 404, description: `Usuário inexistente!` })
  @ApiResponse({
    status: 417,
    description: `Erro ao criar token de recuperação de usuário!`,
  })
  @ApiResponse({ status: 500, description: 'Erro interno do servidor' })
  @ApiOperation({
    summary: 'Envia e cria o token de recuperação ao email do usuário',
  })
  @Public()
  ...
  @ApiResponse({
    status: 401,
    description: `O token enviado não corresponde ao enviado ao email! Tente novamente com outro token de redefinição.`,
  })
  @ApiResponse({
    status: 404,
    description:
      'Usuário não encontrado ou Token inexistente ou o token está expirado!!',
  })
  @ApiResponse({ status: 500, description: 'Erro interno do servidor' })
  ...
  @ApiResponse({
    status: 404,
    description:
      'Usuário não encontrado ou Token inexistente ou o token está expirado ou o Token não está associado ao usuário especificado!',
  })
  @ApiResponse({
    status: 417,
    description: `Ocorreu um erro ao atualizar a senha do usuário!`,
  })
  @ApiResponse({ status: 500, description: 'Erro interno do servidor' })
